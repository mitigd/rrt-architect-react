...
export const THEME = {
  ...
  // Forced dark until I add a toggle -- svelte -> react
...
export default function RftArchitect() {
  ...
  // Wrap handleAnswer in useCallback to prevent stale state issues
  ...
  // KEYBOARD LISTENER - Strictly depends on [isYesRight] to sync keys with buttons
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 w-full h-full max-w-[90vw] bg-slate-950 transition-colors duration-300 font-sans selection:bg-blue-500/30 flex flex-col overflow-hidden">
              ...
              {phase === "RESULT" && (
                <div className="w-full flex flex-col items-center justify-center h-full space-y-3">
                  ...
                  {currentRoundMode.includes("SPATIAL") && currentVisualMap.length > 0 && (
                    <div className="h-48 md:h-64 aspect-square rounded-xl border relative overflow-hidden shrink-0 bg-slate-900" style={{ borderColor: isDark ? THEME.dark.grid : THEME.light.grid }}>
